...
class WeatherService {
  ...
  Stream<Map<String, dynamic>> get weatherStream {
    return _weatherRef.onValue.map((event) {
      ...
      return {
        // Clés Firebase → variables internes de l'application
        'temperature':   (data['temperature']   as num?)?.toDouble(), // Firebase: temperature
        'humidity':      (data['humidite']      as num?)?.toDouble(), // Firebase: humidite
        'pressure':      (data['pression']      as num?)?.toDouble(), // Firebase: pression
        'windSpeed':     (data['vitesseVent']   as num?)?.toDouble(), // Firebase: vitesseVent
        'windDirection': data['directionVent']  as String?,           // Firebase: directionVent
        'rain':          (data['quantitePluie'] as num?)?.toDouble(), // Firebase: quantitePluie
        'latitude':      (data['latitude']      as num?)?.toDouble(), // Firebase: latitude
        'longitude':     (data['longitude']     as num?)?.toDouble(), // Firebase: longitude
        'altitude':      (data['altitude']      as num?)?.toDouble(), // Firebase: altitude
      };
    });
  }
