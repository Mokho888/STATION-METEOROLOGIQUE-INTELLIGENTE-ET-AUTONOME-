...
class _StatsScreenState extends State<StatsScreen> {
  ...
  /// Exporte tous les enregistrements vers un fichier Excel et le télécharge.
  void _exportExcel() {
    ...
    ExcelExportService.exportToExcel(_allRecords);
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(
        content: Row(
          children: [
        ...
        children: [
          ...
          Column(
            ...
            children: [
              ...
              Text('${_allRecords.length} enregistrements · toutes les 30 min',
                  style: GoogleFonts.outfit(
          ...
          children: [
            Row(
              children: [
                ...
                const Spacer(),
                Text(
                  DateFormat('dd MMM yyyy HH:mm', 'fr_FR').format(last.timestamp),
                  style: GoogleFonts.outfit(
            ...
            Wrap(
              ...
              runSpacing: 8,
              children: [
                _valueBadge('${last.temperature.toStringAsFixed(1)}°C',
                    AppTheme.tempColor, Icons.thermostat_rounded),
                _valueBadge('${last.humidity.toStringAsFixed(1)}%',
                    AppTheme.humColor, Icons.water_drop_rounded),
                if (last.pressure != null)
                  _valueBadge('${last.pressure!.toStringAsFixed(0)} hPa',
                      AppTheme.pressColor, Icons.speed_rounded),
                if (last.windSpeed != null)
                  _valueBadge('${last.windSpeed!.toStringAsFixed(1)} km/h',
                      AppTheme.windColor, Icons.air_rounded),
                if (last.rain != null)
                  _valueBadge('${last.rain!.toStringAsFixed(1)} mm',
                      AppTheme.rainColor, Icons.grain_rounded),
                if (last.waterLevel != null)
                  _valueBadge('${last.waterLevel!.toStringAsFixed(1)} cm',
                      AppTheme.waterColor, Icons.waves_rounded),
              ],
            ),
          ],
        ),
          ...
          Row(
            children: [
              _statCard('T° Min',
                  ...
                  const Color(0xFF1976D2), AppTheme.tempColor),
              const SizedBox(width: 8),
              _statCard('T° Max',
                  '${tempStats['max']!.toStringAsFixed(1)}°C',
                  Icons.arrow_upward_rounded,
                  const Color(0xFFE53935), AppTheme.tempColor),
              const SizedBox(width: 8),
              _statCard('T° Moy',
                  '${tempStats['avg']!.toStringAsFixed(1)}°C',
                  Icons.show_chart_rounded,
                  const Color(0xFF388E3C), AppTheme.tempColor),
            ],
          ),
          const SizedBox(height: 8),
          Row(
            children: [
              _statCard('H% Min',
                  ...
                  Icons.arrow_downward_rounded,
                  const Color(0xFF1976D2), AppTheme.humColor),
              const SizedBox(width: 8),
              _statCard('H% Max',
                  '${humStats['max']!.toStringAsFixed(1)}%',
                  Icons.arrow_upward_rounded,
                  const Color(0xFFE53935), AppTheme.humColor),
              const SizedBox(width: 8),
              _statCard('H% Moy',
                  '${humStats['avg']!.toStringAsFixed(1)}%',
                  Icons.show_chart_rounded,
                  const Color(0xFF388E3C), AppTheme.humColor),
            ],
          ),
          ...
          Padding(
            ...
            child: Row(
              children: [
                ...
                const SizedBox(width: 8),
                Expanded(
                  child: Text('Historique récent (30 derniers)',
                      style: GoogleFonts.outfit(
                          color: AppTheme.textSecondary,
                          fontSize: 13,
                          fontWeight: FontWeight.w600)),
