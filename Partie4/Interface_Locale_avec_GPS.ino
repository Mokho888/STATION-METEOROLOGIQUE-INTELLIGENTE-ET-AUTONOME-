...
// ── PARTIE 4 — GPS NEO-6M ────────────────────────────────
// Ces variables sont mises à jour automatiquement dans loop()
float latGPS     = 0.0;
float lonGPS     = 0.0;
char  altGPS[10] = "---";
int   satGPS     = 0;       // nombre de satellites (debug)
...
// Timestamp dernière mise à jour GPS affichée
unsigned long dernierAffichageGPS = 0;
...
void loop() {
  // ── 1. Lecture GPS (non-bloquante) ──────────────────────
  while (GPS_Serial.available() > 0) {
    gps.encode(GPS_Serial.read());
  }

  // ── 2. Mise à jour des variables GPS toutes les 2s ──────
  if (millis() - dernierAffichageGPS > 2000) {
    dernierAffichageGPS = millis();
    ...
    // Rafraîchir la page 3 si le GPS a changé et qu'elle est active
    if (gpsChange && pageActuelle == 3) {
      afficherPage(3);
    }

    // Debug série
    Serial.print("SAT:"); Serial.print(satGPS);
    Serial.print("  Lat:"); Serial.print(latGPS, 6);
    Serial.print("  Lon:"); Serial.print(lonGPS, 6);
    Serial.print("  Alt:"); Serial.println(altGPS);
  }

  // ── 3. Gestion du tactile ───────────────────────────────
  if (ts.touched()) {
...
// Carte GPS dédiée — affiche position + indicateur de fix
void dessinerCarteGPS(int y_start, float lat, float lon, const char* alt) {
  ...
  // Couleur indicateur : vert si fix, rouge sinon
  uint16_t colFix = gps.location.isValid() ? ILI9341_GREEN : ILI9341_RED;

  tft.fillRoundRect(x, y, w, h, 8, C_CARD);
  tft.drawRoundRect(x, y, w, h, 8, col);
  tft.fillRoundRect(x+8, y, w-16, 3, 1, col);
  drawIcon(9, x+30, y + h/2, colFix);   // icône GPS colorée selon le fix
  ...
  if (gps.location.isValid()) {
    // ── Fix obtenu : afficher les coordonnées ──
    tft.setTextColor(C_WHITE);
    tft.setTextSize(2);
    ...
    // ── Pas encore de fix : message d'attente ──
